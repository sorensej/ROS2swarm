...
class ChainSearchPattern(MovementPattern):

    def __init__(self):
        """Initialize the minimalist flocking pattern node."""
        super().__init__('chain_search_pattern')

        self.declare_parameters(
            namespace='',
            parameters=[
                ('minimalist_flocking_translational_velocity', Parameter.Type.DOUBLE),
                ('minimalist_flocking_rotational_left_velocity', Parameter.Type.DOUBLE),
                ('minimalist_flocking_rotational_right_velocity', Parameter.Type.DOUBLE),
                ('minimalist_flocking_drive_timer_period', Parameter.Type.INTEGER),
                ('minimalist_flocking_zone1_threshold', Parameter.Type.DOUBLE),
                ('minimalist_flocking_zone2_threshold', Parameter.Type.DOUBLE),
                ('minimalist_flocking_zone3_threshold', Parameter.Type.DOUBLE),
                ('minimalist_flocking_zone4_threshold', Parameter.Type.DOUBLE),
                ('minimalist_flocking_zone2_robot_threshold', Parameter.Type.INTEGER),
                ('minimalist_flocking_robot_threshold', Parameter.Type.INTEGER),
        ...
        # set gradient broadcast message
        msg = Int8Message()
        ...
        self.value_publisher = self.create_publisher(Int8Message,
                                            self.get_namespace() + '/value',
                                            self.value._data)
    ...
    def scan_callback(self, incoming_msg):
        ...
        # Broadcast the value of the agent
        self.value_publisher.publish(self.value)

        # only non-nest nodes move
        if self.state != State.NEST:
    ...
    def vector_calc(self):
        ...
        if self.state is State.AVOID or (self.state is State.EXPLORE and
        ...
        # zone 2: separation
                ...
                self.state is State.EXPLORE and (obstacle[0] or obstacle[1] or obstacle[2])):
            behave = self.separation()
            self.state = State.DISPERSION

        # zone 3: attraction/cohesion
        elif self.state is State.ATTRACTION or (self.state is State.EXPLORE and
        ...
        elif self.state is State.ATTRACTION_BACK or (self.state is State.EXPLORE and
                                                     check_robot(self.param_max_range,
                                                                 self.param_zone4_threshold,
                                                                 self.ranges[Directions.BEHIND],
                                                                 self.param_robot_threshold)):
            self.state = State.CHAIN_NODE
...
def main(args=None):
    ...
    setup_node.init_and_spin(args, ChainSearchPattern)
